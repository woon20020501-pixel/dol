...
- **NAV reporting dependence.** The Vault's reported NAV is updated by an off-chain operator (currently the bot). If the operator is offline, slow, or compromised, the on-chain NAV used to compute pricePerShare can become stale. Withdrawals during stale-NAV periods may execute at incorrect prices.
